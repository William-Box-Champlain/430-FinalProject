...
Socket Socket::Accept() {
  ...
  if (connection == INVALID_SOCKET) {
    ...
    if (error == WSAEWOULDBLOCK) {
      ...
      _last_error = SOCKLIB_ETIMEDOUT; // Reuse timeout error code
      Socket empty_sock;
      return empty_sock;
    }
    // For any other error, use the require macro to abort
...
int Socket::Connect(const Address &address) {
  ...
  int result = connect(to_native_socket(*this), (sockaddr *)&native_addr, sizeof(native_addr));
  if (result == SOCKET_ERROR) {
    int error = WSAGetLastError();
    if (error == WSAEWOULDBLOCK || error == WSAEALREADY || error == WSAEINPROGRESS) {
      // Non-blocking socket would block, this is normal
      _last_error = SOCKLIB_ETIMEDOUT; // Reuse timeout error code
      return -1;
    }
    else if (error == WSAEISCONN) {
      // Socket is already connected, this is fine
      return 0;
    }
    // For any other error, use the require macro to abort
    require(result != SOCKET_ERROR, "connect()");
  }
...
size_t Socket::Send(const char *data, size_t len) {
    ...
    if (count == SOCKET_ERROR) {
        ...
        throw std::runtime_error(std::string("send(): ") + std::to_string(error));
    }
    return count;
}
